...
def _collect_pdbs_from_af2_dir(config: dict[str, Any]) -> tuple[pd.DataFrame, str]:
    ...
    This function keeps that source-of-truth behavior inside CatConGraph.
    """
...
def _resolve_template_and_ligands(config: dict[str, Any]) -> tuple[Path, list[Path]]:
    ...
    in the desired docking order.  CatConGraph now mirrors the user's sequential
...
def _load_ligands(ligand_pdbs: list[Path]) -> list[Chem.Mol]:
    ...
    for ligand_pdb in ligand_pdbs:
        ...
        if mol is None:
            raise ValueError(f"无法加载小分子/辅因子/金属文件: {ligand_pdb}")
        if mol.GetNumConformers() == 0:
            try:
                AllChem.EmbedMolecule(mol, randomSeed=42)
                print(f"✅ 为 {ligand_pdb.name} 生成了3D构象", flush=True)
            except Exception as exc:
                raise ValueError(f"无法为 {ligand_pdb} 生成所需的3D构象") from exc
        mols.append(mol)
    return mols
...
def run_step02(
    ...
    *,
    ...
    print("🚀 加载模板蛋白和小分子/辅因子/金属构象...", flush=True)
    ...
    if len(ligand_pdbs) > 1:
        print(
            f"✅ 已加载 {len(ligand_pdbs)} 个非蛋白实体文件；"
            f"将按用户给定顺序执行 {multi_ligand_mode} 多底物/辅因子对接",
            flush=True,
        )
    ...
    for out_i, (_, row) in enumerate(structure_df.iterrows(), start=1):
        ...
        print(f"\n📦 处理突变体结构: {source_pdb.name}", flush=True)
        ...
        try:
            ...
            print(f"✅ 复合物已保存: {output_complex}", flush=True)
        except Exception as exc:
            alignment_audit = {}
            result_row = {
                "input_order": int(row.get("input_order", out_i)),
...
def main(argv: list[str] | None = None) -> None:
    ...
    print("[CatConGraph] Step 02 completed.")
