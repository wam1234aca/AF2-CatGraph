...
try:
    ...
    def tqdm(iterable, **kwargs):
        print("Note: tqdm is not installed. Install it with 'pip install tqdm' to display progress bars.")
        return iterable
...
class EnzymeSubstrateVisualizer:
    ...
    def _parse_file_lines(self, path: str) -> List[str]:
        try:
            ...
            logging.error(f"Could not parse file {path}: {e}")
            return []
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Generate representative interaction graphs for GED classes.", formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    
    io_group = parser.add_argument_group('Input/Output Files')
    io_group.add_argument("-i", "--input", required=True, type=Path, help="GED class directory containing class_* subfolders.")
    io_group.add_argument("-p", "--plip_dir", required=True, type=Path, help="Raw PLIP data directory, such as surviving_plip_outputs.")
    io_group.add_argument("-o", "--output", required=True, type=Path, help="Directory for generated figures.")
    io_group.add_argument("-f", "--format", default="svg", choices=["svg", "png", "pdf"], help="Output figure format.")

    micromanagement_group = parser.add_argument_group('Micro-management (for first class only)')
    micromanagement_group.add_argument("--swap_substrates", action='store_true', help="Swap the positions of the two largest substrates.")
    micromanagement_group.add_argument("--rotate_substrate", nargs=2, type=float, action=RotateSubstrateAction, metavar=('<N>', '<ANGLE>'), help="Rotate substrate N by ANGLE degrees.")
    micromanagement_group.add_argument("--rotate", type=float, default=0.0, help="Apply a global rotation to every figure.")

    fig_group = parser.add_argument_group('Figure Styling')
    fig_group.add_argument('--fig_width', type=float, default=65.643/25.4, help="Figure width in inches.")
    fig_group.add_argument('--fig_height', type=float, default=49.488/25.4, help="Figure height in inches.")
    fig_group.add_argument('--dpi', type=int, default=300, help="Figure resolution in DPI.")

    node_group = parser.add_argument_group('Node Styling')
    node_group.add_argument('--node_size', type=int, default=450, help="Residue node size.")
    node_group.add_argument('--ligand_node_color', type=str, default='skyblue', help="Ligand-atom node color.")
    node_group.add_argument('--protein_node_color', type=str, default='#E0E0E0', help="Protein-residue node color.")
    node_group.add_argument('--node_border_color', type=str, default='black', help="Node border color.")

    edge_group = parser.add_argument_group('Edge Styling')
    edge_group.add_argument('--covalent_edge_width', type=float, default=0.8, help="Covalent-edge width.")
    edge_group.add_argument('--interaction_edge_width', type=float, default=1.0, help="Interaction-edge width.")
    edge_group.add_argument('--edge_alpha', type=float, default=0.8, help="Edge opacity.")
    edge_group.add_argument('--hbond_edge_color', type=str, default=INTERACTION_COLORS['hydrogen_bonds'], help="Hydrogen-bond color.")
    edge_group.add_argument('--saltbridge_edge_color', type=str, default=INTERACTION_COLORS['salt_bridges'], help="Salt-bridge color.")
    edge_group.add_argument('--hydrophobic_edge_color', type=str, default=INTERACTION_COLORS['hydrophobic_interactions'], help="Hydrophobic-interaction color.")
    edge_group.add_argument('--pistack_edge_color', type=str, default=INTERACTION_COLORS['pi_stacks'], help="Pi-stacking color.")

    font_group = parser.add_argument_group('Font Styling')
    font_group.add_argument('--font_size', type=int, default=6.5, help="Label font size.")
    font_group.add_argument('--font_color', type=str, default='black', help="Label font color.")
    font_group.add_argument('--font_weight', type=str, default='bold', help="Label font weight: normal or bold.")
    
    debug_group = parser.add_argument_group('Debugging')
    debug_group.add_argument("-v", "--verbose", action="store_true", help="Enable detailed console diagnostics.")
