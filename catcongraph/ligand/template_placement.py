...
def build_template_to_target_transform(
    *,
    ...
    This is the same core geometry as the user's original docking script: the AF2
...
def avoid_ligand_collision_deterministic(
    *,
    ...
    mirrors the original script's strategy: first small translations, then small
...
def place_template_ligands_for_one_structure(
    *,
    ...
    try:
        ...
        remark_lines = [
            f"CatConGraph Step02 sequential template-guided rigid docking for {standard_structure_id}",
        ...
        if output_aligned_protein_pdb:
            ...
            _write_atom_subset_with_coords(
                ...
                remark_lines=[f"CatConGraph Step02 aligned protein for {standard_structure_id}"],
            )
        ...
        if output_placed_ligand_pdb:
            ...
            with output_placed_ligand_pdb.open("w", encoding="utf-8") as out:
                serial = 1
                out.write(f"REMARK CatConGraph Step02 placed ligand groups for {standard_structure_id}\n")
