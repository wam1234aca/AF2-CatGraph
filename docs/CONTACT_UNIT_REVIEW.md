...
applied. GAC, AqTMK, and PTP1B use the same detector as every other enzyme;
their Tables S6, S10, and S14 mappings are checked as regression benchmarks,
not loaded as target-specific runtime presets.
...
## Guided review (legacy YAML compatibility)

Set `contact_unit_mode: guided_review` to generate reviewable proposals:

- known ligands may match built-in reviewed definitions;
- other ligands may generate proposals from functional groups, PDB `CONECT`
  records or strict local connectivity, and recurrence across the ensemble;
- a proposal affects classification only after it is accepted, saved to the
  project YAML, and Step08 is rerun;
- `manual_ligand_units` can add or override explicit definitions.

This route is not shown in the compact GUI. Existing YAML projects can retain
it unchanged. The legacy procedure is:

1. Complete Step01–Step07.
2. Keep `contact_unit_mode: guided_review` in the project YAML.
3. Preview the proposals and inspect ligand residue names, atom names,
   connectivity evidence, and contact evidence.
4. Accept or reject proposals and save the review decisions and project YAML.
5. Rerun Step08:

```bash
af2-catgraph step08 --config projects/MY_ENZYME/config.yaml --clean
```

Step08 writes the following audit tables:

- `*_step08_substrate_inventory.csv`: ligand residues and atoms found in the
  analysed structures;
- `*_step08_ligand_unit_recommendations.csv`: proposal status and supporting
  evidence;
- `*_step08_raw_to_canonical_mapping.csv`: canonical contact label used for
  each raw PLIP record;
- `*_step08_ligand_unit_coverage_audit.csv`: coverage of manual rules and
  accepted proposals against observed PLIP ligand atoms.
...
|---|---|
| Two oxygen atoms attached to the same carboxyl/carboxylate center | A proposal may be considered after checking stereospecific contacts and mechanistic context. |
| Non-bridging oxygen atoms attached to the same phosphate center | A proposal may be considered; do not group bridging oxygen atoms, atoms from different phosphate centers, or the central phosphorus by name alone. |
...
Before a full Step08 run, inspect proposals and mapping audits from a small
representative subset of structures. Retain accepted and manual mappings in the
project YAML used for the final analysis.
