...
def test_gui_is_english_only_and_keeps_headings_unclipped() -> None:
    ...
    assert "ccg_language" not in state_source
    assert "Language /" not in main_source
    assert not re.search(r"[\u3400-\u9fff]", gui_source)
...
def test_run_step_labels_cover_every_choice() -> None:
    fake_st = SimpleNamespace(session_state={})
    labels = gui._step_labels(fake_st)

    assert set(gui.STEP_KEYS) <= set(labels)
    assert all(isinstance(labels[key], str) and labels[key] for key in gui.STEP_KEYS)
